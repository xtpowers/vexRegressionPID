...
def der_sigmoid_distance(x, y):
    ...
    e = 2.718281828459045
    return y - (10 * math.pow(e, 10 * x + 5)) / (
                (math.pow(e, 10 * x) + math.pow(e, 5)) * (math.pow(e, 10 * x) + math.pow(e, 5)))
...
def reg_degree_pid(degrees, intended_time, min_err, kp, kd, ki):
    ...
    while (abs(err) > min_err and cur_time < max_time):
        ...
        yi += (yp + last_yp) * 0.05
        cout = kp * sigmoid_distance(x, yp) + kd * der_sigmoid_distance(x, yd) * 0.4 + ki * int_sigmoid_distance(x,
                                                                                                                 yi) * 2
        if (abs(cout) > 3.3):
...
def user_control():
    ...
    while (True):
        ...
        turn = controller.axis1.value() * .12 * .7
        ...
        if abs(drive) + abs(turn) < .25:
            ...
            left_motors.spin(FORWARD, drive + turn, VoltageUnits.VOLT)
            right_motors.spin(FORWARD, drive - turn, VoltageUnits.VOLT)
